...
Emaitza  VARCHAR(30);
BEGIN
IF (MOD(Zenbakia,Bikoitia)) = 0 THEN
...
set SERVEROUTPUT ON
DECLARE
TIPO_PRODUKTU  char(1) := UPPER('A');

Emaitza  VARCHAR(30);
BEGIN
IF (TIPO_PRODUKTU = 'A') THEN
DBMS_OUTPUT.PUT_LINE('Elektronika');
elsif (TIPO_PRODUKTU = 'B') then
DBMS_OUTPUT.PUT_LINE('Informatika');
elsif (TIPO_PRODUKTU = 'C') then
DBMS_OUTPUT.PUT_LINE('Musika');
elsif (TIPO_PRODUKTU = 'D') then
...
SET SERVEROUTPUT ON
 declare
 Testua varchar2(100):='Kaixo Zer moduz';
 Emaitza varchar2(100);
 x  number; 
 begin
 x := length(Testua); 
 while x <> 0 loop
 Emaitza := Emaitza || substr(Testua,x,1); --Testua alderantziz idatzi
 x := x -1;
 ...
 x := length(Testua); 
 while x <> 0 loop
 Emaitza := Emaitza || substr(Testua,x,1);
 x := x -1;
 EXIT WHEN INSTR(Emaitza, 'x' or 'X') > 0; --Konprobatu ea Testuak X letra duen
